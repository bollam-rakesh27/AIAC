...
def quick_sort(arr, key=lambda x: x, reverse=False):
    ...
    pivot = arr[len(arr)//2]
    if reverse:
        left=[x for x in arr if key(x)>key(pivot)]; mid=[x for x in arr if key(x)==key(pivot)]; right=[x for x in arr if key(x)<key(pivot)]
    else:
        left=[x for x in arr if key(x)<key(pivot)]; mid=[x for x in arr if key(x)==key(pivot)]; right=[x for x in arr if key(x)>key(pivot)]
...
if __name__ == "__main__":
    ...
    t0=time.time(); sorted_quick=quick_sort(students, key=lambda x:x[2], reverse=True); print(f"Quick Sort Time: {time.time()-t0:.4f} seconds")
    t0=time.time(); sorted_merge=merge_sort(students, key=lambda x:x[2], reverse=True); print(f"Merge Sort Time: {time.time()-t0:.4f} seconds")
